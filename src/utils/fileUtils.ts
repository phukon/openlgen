...
export class FileUtils {
  static async updatePackageJson({ license, author }: PackageJsonUpdate): Promise<boolean> {
  ...
  static async writeLicenseFile(content: string): Promise<boolean> {
    try {
      ...
      throw new Error(`Failed to write LICENSE file: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
}